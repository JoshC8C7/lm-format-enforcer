...
class JsonFreetextTokenCache:
    ...
    def __init__(self, ) -> None:
        ...
        self.regular_tokens_length_cache = StringLengthTokenCache()
        self.quote_tokens_length_cache = StringLengthTokenCache()
    ...
    def add_token(self, token_str: str, token_int: int):
        ...
        if has_non_trailing_backslash or has_quote_before_end or has_newline:
            try:
                ...
                print("was fine: ", [token_str])


        #todo com1 what do we do about tokens like fish"} - they are technically legal here! - putting it in the strings like that is artificially constraining it.
...
class ValueStartCache:
    ...
    def freeze(self) -> None:
        ...
        self.tokens_length_cache.build(all_tokens)
        self.max_token_len = len(self.tokens_length_cache.first_index_geq_than_length)
        del self.token_num_to_str
...
class TokenizerPrefixTree:
    def __init__(self, regular_tokens: List[Tuple[int, str, bool]]):
        ...
        self.value_start_tokens = ValueStartCache()
